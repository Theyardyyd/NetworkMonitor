                ...
                try
                {
                    ...
                    using (_session = new TraceEventSession(sessionName))
                    {
                        ...
                        Action<string, string, ulong, int, bool> ProcessPacket = (src, dst, size, pid, isUpload) =>
                        {
                            ...
                            int targetPid = pid;
                            // 穿透 System(4) 与 Idle(0) 获取真实的套接字归属 PID
                            if ((pid == 0 || pid == 4) && IpToPidMap.TryGetValue(remoteIp, out int realPid))
                            {
                                targetPid = realPid;
                            }

                            // ★ 修复 Bug 2：剥离了错误的 else 分支，并严格根据 isUpload 标志位将流量分发到各自的账本中
                            if (isUpload)
                            {
                                ProcessUploadBytes.AddOrUpdate(targetPid, size, (id, old) => old + size);
                            }
                            else
                            {
                                ProcessDownloadBytes.AddOrUpdate(targetPid, size, (id, old) => old + size);
                            }
