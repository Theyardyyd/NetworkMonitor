...
namespace NetworkMonitor
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void MainCanvas_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            ...
            // ★ 修复 Bug 0：在鼠标按下时立即冻结流动窗口
            _isChartDragPaused = true;
            _chartPauseTime = DateTime.Now;
            _hasRenderedPausedState = false;
        ...
        private void MainCanvas_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            if (_isDraggingChart)
            {
                ...
                if (rightX - leftX > 5) // 防误触，拉出有效范围则截取时间
                {
                    ...
                    // ★ 修复 Bug 0：只有在用户确定松开鼠标且框选有效时，才冻结流动窗口
                    _isChartDragPaused = true;
                    _chartPauseTime = DateTime.Now;

                    _hasRenderedPausedState = false; // 开放一次渲染权限以截取该时段数据
                    ...
                    if (PanelProcessLoading != null) PanelProcessLoading.Visibility = Visibility.Visible;
                    Dispatcher.InvokeAsync(() => ProcessTimer_Tick(null, null), DispatcherPriority.Background);
                }
                else
                {
                    ...
                    // 恢复实时状态时，隐藏选区时间范围文本
                    if (TxtSelectedTimeRange != null) TxtSelectedTimeRange.Visibility = Visibility.Collapsed;
                    if (PanelProcessLoading != null) PanelProcessLoading.Visibility = Visibility.Visible;
        ...
        private ConcurrentDictionary<string, HashSet<string>> _processIpCache = new ConcurrentDictionary<string, HashSet<string>>(); // ★ 新增：跨周期保留进程的历史通信 IP
        private ConcurrentDictionary<int, long> _pidLastRecordedDaily = new ConcurrentDictionary<int, long>();
        
        // ★ 修复 Bug：新增冻结时间锚点和真正记录每个 IP 实际流量分摊的账本
        private DateTime _chartPauseTime = DateTime.MinValue;
        private bool _hasRenderedPausedState = false;
        private ConcurrentDictionary<int, (ulong Read, ulong Write)> _pidLastRW = new ConcurrentDictionary<int, (ulong Read, ulong Write)>();
        // 使用明确命名的元组字典
        private ConcurrentDictionary<string, ConcurrentDictionary<string, (long Down, long Up)>> _appIpTraffic = new ConcurrentDictionary<string, ConcurrentDictionary<string, (long Down, long Up)>>();
        // 用于实时计算图表的缓存快照
        ...
        // ★ 修复资源历史模式下的拉伸截断Bug
        private void ResCanvas_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            if (_activeTab == "Resources" && BtnBackToLive != null && BtnBackToLive.Visibility == Visibility.Visible && !string.IsNullOrEmpty(_currentHistoryDate))
            {
                if (TimeSpan.TryParse(TxtHistoryStart.Text, out TimeSpan start) && TimeSpan.TryParse(TxtHistoryEnd.Text, out TimeSpan end))
                {
                    DrawStaticResourceLines(_currentHistoryDate, start, end);
                }
            }
        }

        // ★ 修复 Bug 2：利用原生事件在行展开瞬间立即抓取基础进程信息，并直接操作 UI 突破模型绑定的限制
        private void ProcessGrid_LoadingRowDetails(object sender, DataGridRowDetailsEventArgs e)
        {
            ...
            {
                // ★ 动态抓取 UI 元素引用
                var border = e.DetailsElement as Border;
                var txtPath = border?.FindName("TxtProcessPath") as TextBlock;
                var txtVer = border?.FindName("TxtProcessVersion") as TextBlock;
                var txtPub = border?.FindName("TxtProcessPublisher") as TextBlock;
                ...
                Task.Run(() => {
                    try
                    {
                        ...
                        if (procs.Length > 0)
                        {
                            ...
                            Application.Current.Dispatcher.InvokeAsync(() => {
                                if (txtPath != null) txtPath.Text = "路径: " + path;
                                if (txtVer != null) txtVer.Text = "版本: " + (ver.FileVersion ?? "未知");
                                if (txtPub != null) txtPub.Text = "发布者: " + (ver.CompanyName ?? "未知");
                            });
                        }
                        else
                        {
                            Application.Current.Dispatcher.InvokeAsync(() => {
                                if (txtPath != null) txtPath.Text = "路径: 进程已退出或被系统隐藏，无法获取";
                                if (txtVer != null) txtVer.Text = "版本: 未知";
                                if (txtPub != null) txtPub.Text = "发布者: 未知";
                            });
                        }
                    }
                    catch
                    {
                        Application.Current.Dispatcher.InvokeAsync(() => {
                            if (txtPath != null) txtPath.Text = "路径: 权限不足，无法访问此系统级或受保护进程";
                            if (txtVer != null) txtVer.Text = "版本: 权限受限";
                            if (txtPub != null) txtPub.Text = "发布者: 权限受限";
                        });
                    }
                });
            }
        ...
        // ★ 修复 Bug 3：真正调用 API 获取 IP 位置，并规避不触发刷新的问题
        private async void BtnShowIPs_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button btn && btn.DataContext is ProcessNetworkInfo rowData)
            {
                ...
                var tempConnections = rowData.Connections.ToList();

                // 核心修复：使用并发 Task.WhenAll 解决查询 IP 归属地耗时过长的问题，避免 foreach 阻塞
                var fetchTasks = tempConnections.Select(async conn =>
                {
                    if (conn.IP == "127.0.0.1" || conn.IP == "0.0.0.0" || conn.IP.StartsWith("192.168.") || conn.IP.StartsWith("10.") || conn.IP.StartsWith("172."))
                    {
                        conn.Region = "局域网 (LAN) / 本地";
                    }
                    else
                    {
                        var geo = await GetGeoInfo(conn.IP);
                        if (!string.IsNullOrEmpty(geo.City) || !string.IsNullOrEmpty(geo.Country))
                            conn.Region = $"{geo.Country}, {geo.City}";
                        else
                ...
                // ★ 强制清空并重新填充，解决 ObservableCollection 内对象属性修改不刷新的问题！
                rowData.Connections.Clear();
                foreach (var c in tempConnections) rowData.Connections.Add(c);
        ...
        // ★ 修复 Bug 1：防止图表拉伸时选择框悬空，并彻底修复暂停期间窗口缩放导致的坐标轴截断问题
        private void MainCanvas_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            double newW = e.NewSize.Width;
            double newH = e.NewSize.Height;
            double oldW = e.PreviousSize.Width;
            double oldH = e.PreviousSize.Height;

            if (oldW == 0 || oldH == 0) return;

            if (ChartSelectionRect != null && ChartSelectionRect.Visibility == Visibility.Visible && !_isDraggingChart)
            {
                double leftX = newW * (1 - _selectedTimeEndRatio); 
                double rightX = newW * (1 - _selectedTimeStartRatio);
                if (leftX < 0) leftX = 0;
                if (rightX > newW) rightX = newW;
                Canvas.SetLeft(ChartSelectionRect, leftX);
                ChartSelectionRect.Width = Math.Max(0, rightX - leftX);
                ChartSelectionRect.Height = newH;
                Canvas.SetTop(ChartSelectionRect, 0);
            }

            // ★ 完美修复：如果当前处于冻结暂停状态，绝对不能再查活数据，而是对内存里现存的多边形进行等比例纯图形学拉伸缩放！
            if (_isChartPaused && _activeTab == "Traffic" && newW > 0 && oldW > 0)
            {
                double scaleX = newW / oldW;
                double scaleY = newH / oldH;

                ScalePolygon(PolyDown, scaleX, scaleY, oldW, oldH, newW, newH);
                ScalePolygon(PolyUp, scaleX, scaleY, oldW, oldH, newW, newH);

                for (int i = 0; i < _gridLines.Count; i++)
                {
                    var ln = _gridLines[i];
                    if (ln.Visibility == Visibility.Visible)
                    {
                        ln.X1 *= scaleX;
                        ln.X2 *= scaleX;
                        ln.Y2 = newH;
                        if (i < _timeLabels.Count)
                        {
                            Canvas.SetLeft(_timeLabels[i], Canvas.GetLeft(_timeLabels[i]) * scaleX);
                        }
                    }
                }

                foreach (var kvp in _chartEventMarkers)
                {
                    var marker = kvp.Value;
                    Canvas.SetLeft(marker, Canvas.GetLeft(marker) * scaleX);
                    
                    // ★ 修复 CS1061 错误：将 UIElement 强转为 FrameworkElement 以访问 Height 属性
        ...
        // 辅助算法：冻结帧完美等比例拉伸，防止底部边缘漏风
        private void ScalePolygon(Polygon poly, double scaleX, double scaleY, double oldW, double oldH, double newW, double newH)
        {
            if (poly == null || poly.Points == null) return;
            var newPts = new PointCollection(poly.Points.Count);
            foreach (var p in poly.Points)
            {
                double nx = p.X;
                double ny = p.Y;

                // X轴处理：如果是屏幕外的填充锚点，依然推离屏幕外
                if (nx < -50) nx = -200;
                else if (nx >= oldW + 50) nx = newW + 200;
                else nx *= scaleX;

                // Y轴处理：如果是贴着旧屏幕底部的点，自动吸附到新屏幕底部
                if (ny >= oldH - 0.5) ny = newH;
                else ny *= scaleY;

                newPts.Add(new Point(nx, ny));
            }
            poly.Points = newPts;
        }
        ...
        private Dictionary<string, long> AggregateMinuteData(Dictionary<string, Dictionary<string, long>> db, string dateStr, TimeSpan start, TimeSpan end, Dictionary<string, Dictionary<string, long>> fallbackDailyDb)
        {
            ...
            int endSec = (int)end.TotalSeconds;
            ...
            for (int s = startSec; s <= endSec; s++)
            {
                ...
                string sKey = $"{dateStr}_{h:D2}{m:D2}{sec:D2}";   // 最新版: 143015 (精确到秒)
                string mKey = $"{dateStr}_{h:D2}{m:D2}";           // 旧版: 1430
                string oldHKey = $"{dateStr}_{h:D2}";              // 更老版: 14

                if (db.TryGetValue(sKey, out var secData))
                {
                    hasData = true;
                    foreach (var kv in secData) result[kv.Key] = result.GetValueOrDefault(kv.Key, 0) + kv.Value;
                }
                // 兼容旧版保存的分钟级与小时级数据，当且仅当循环步进到对应刻度时探测
                else if (sec == 0 && db.TryGetValue(mKey, out var minData))
                {
                    hasData = true;
                    foreach (var kv in minData) result[kv.Key] = result.GetValueOrDefault(kv.Key, 0) + kv.Value;
                }
                else if (sec == 0 && m == 0 && db.TryGetValue(oldHKey, out var hourData))
                {
                    hasData = true;
                    foreach (var kv in hourData) result[kv.Key] = result.GetValueOrDefault(kv.Key, 0) + kv.Value;
                }
        ...
        {
            // ===== 插入闲置状态检测 =====
            // 使用 GetLastInputInfo 严格检测键鼠物理状态，彻底排除视频全屏等系统 API 层面的干扰
            bool isInactive = IsUserInactive(60);
            ...
            try
            {
                var uiDict = await Task.Run(() => {
                    ...
                    foreach (int pid in activePids)
                    {
                        ...
                        // ★ 新增：计算 2 秒间的真实微分增量，并注入给这一瞬间活跃的通信 IP
                        if (!_pidLastRW.ContainsKey(pid)) _pidLastRW[pid] = (cR, cW);
                        var lastRW = _pidLastRW[pid];
                        // ★ 修复 Bug 1：彻底隔离上传和下载的增量计算
                        ulong tickR = cR >= lastRW.Read ? cR - lastRW.Read : 0;
                        ulong tickW = cW >= lastRW.Write ? cW - lastRW.Write : 0;
                        _pidLastRW[pid] = (cR, cW);
                        ...
                        // ★ 核心修复 Bug 2：将滴答间的真实流量，只累加分配给当前周期内存活的 IP
                        var activeIpsForThisPid = pidIPs.ContainsKey(pid) ? pidIPs[pid].ToList() : new List<string>();
                        if (!_appIpTraffic.ContainsKey(pName)) _appIpTraffic[pName] = new ConcurrentDictionary<string, (long Down, long Up)>();
                        
                        if (activeIpsForThisPid.Count > 0 && (tickR > 0 || tickW > 0))
                        {
                            long avgTickR = (long)(tickR / (ulong)activeIpsForThisPid.Count);
                            long avgTickW = (long)(tickW / (ulong)activeIpsForThisPid.Count);
                            foreach (var ip in activeIpsForThisPid)
                            {
                                var existing = _appIpTraffic[pName].GetValueOrDefault(ip, (0L, 0L));
                                // ★ 统一使用 Item1/Item2 确保绝对兼容性，且不再混淆上传下载
                                _appIpTraffic[pName][ip] = (existing.Item1 + avgTickR, existing.Item2 + avgTickW);
                            }
                        }
                        else if (activeIpsForThisPid.Count == 0 && (tickR > 0 || tickW > 0) && _processIpCache.ContainsKey(pName) && _processIpCache[pName].Count > 0)
                        {
                            // 瞬间捕捉不到 Socket 句柄，但有流量产生，平摊给这程序的历史缓存 IP
                            var histIps = _processIpCache[pName].ToList();
                            long avgTickR = (long)(tickR / (ulong)histIps.Count);
                            long avgTickW = (long)(tickW / (ulong)histIps.Count);
                            foreach (var ip in histIps)
                            {
                                var existing = _appIpTraffic[pName].GetValueOrDefault(ip, (0L, 0L));
                                _appIpTraffic[pName][ip] = (existing.Item1 + avgTickR, existing.Item2 + avgTickW);
                            }
                        ...
                        if (tickDelta > 0)
                        {
                            ...
                            // 升级为秒级粒度存储，精确到每一秒的切片
                            string sKey = $"{dateKey}_{DateTime.Now.Hour:D2}{DateTime.Now.Minute:D2}{DateTime.Now.Second:D2}";
                            ...
                            lock (_saveDataLock)
                            {
                                ...
                                // 替换底层操作使用的字典Key
                                if (!_savedData.HourlyAppTraffic.ContainsKey(sKey)) _savedData.HourlyAppTraffic[sKey] = new Dictionary<string, long>();
                                _savedData.HourlyAppTraffic[sKey][pName] = _savedData.HourlyAppTraffic[sKey].GetValueOrDefault(pName, 0) + tickDelta;
                        ...
                        if (!_processIpCache.ContainsKey(pName)) _processIpCache[pName] = new HashSet<string>();

                        if (pidIPs.ContainsKey(pid))
                        {
                            foreach (var ip in pidIPs[pid]) {
                                pAggs[pName].ConnectedIPs.Add(ip);
                                _processIpCache[pName].Add(ip);
                            }
                        }

                        // ★ 核心修复：如果当前瞬间没抓到套接字，但由于前面判断有流量，从缓存池中提取该进程历史通信过的 IP 来分摊显示流量
                        if (pAggs[pName].ConnectedIPs.Count == 0 && _processIpCache.ContainsKey(pName))
                        {
                            foreach (var ip in _processIpCache[pName]) pAggs[pName].ConnectedIPs.Add(ip);
                        }
                    }
                    ...
                    lock (_saveDataLock)
                    {
                        ...
                        // ★ 若正在拖拽分析，则冻结计算时间的基线（不要使用一直在变动的 DateTime.Now）
                        DateTime now = _isChartDragPaused ? _chartPauseTime : DateTime.Now;
                        Dictionary<string, long> windowTotals = new Dictionary<string, long>();

                        // 根据用户拉拽选择的窗口范围（或默认完整时间），精确提取流量
                        double windowSec = _currentViewMode;
                        ...
                        // ★ 核心修复：嗅探底层实际有数据的精确起止时间 (移到外层以供 UI 如实显示)
                        DateTime? actualDataStart = null;
                        DateTime? actualDataEnd = null;
                        bool isDailyMode = false;

                        if (_currentViewMode <= 86400) // 24小时以内查秒级日志
                        {
                            // 改为精确到秒的迭代起止时间
                            DateTime iter = new DateTime(selectionStart.Year, selectionStart.Month, selectionStart.Day, selectionStart.Hour, selectionStart.Minute, selectionStart.Second);
                            DateTime endIter = new DateTime(selectionEnd.Year, selectionEnd.Month, selectionEnd.Day, selectionEnd.Hour, selectionEnd.Minute, selectionEnd.Second).AddSeconds(1);

                            while (iter < endIter)
                            {
                                string sKey = $"{iter:yyyy-MM-dd}_{iter.Hour:D2}{iter.Minute:D2}{iter.Second:D2}";
                                string mKey = $"{iter:yyyy-MM-dd}_{iter.Hour:D2}{iter.Minute:D2}"; // 兼容旧版分钟级
                                string oldHKey = $"{iter:yyyy-MM-dd}_{iter.Hour:D2}";             // 兼容更老的小时级

                                bool hasDataThisSlice = false; // 标记该时间切片是否有真实数据命中

                                if (_savedData.HourlyAppTraffic.TryGetValue(sKey, out var dict))
                                {
                                    hasDataThisSlice = true;
                                ...
                                else if (_savedData.HourlyAppTraffic.TryGetValue(mKey, out var dictMin))
                                {
                                    hasDataThisSlice = true;
                                    foreach (var kvp in dictMin)
                                    {
                                        targetProcesses.Add(kvp.Key);
                                        windowTotals[kvp.Key] = windowTotals.GetValueOrDefault(kvp.Key, 0) + kvp.Value;
                                    }
                                }
                                else if (iter.Minute == 0 && iter.Second == 0 && _savedData.HourlyAppTraffic.TryGetValue(oldHKey, out var dictOld))
                                {
                                    hasDataThisSlice = true;
                                ...
                                if (hasDataThisSlice)
                                {
                                    if (!actualDataStart.HasValue) actualDataStart = iter;
                                    actualDataEnd = iter;
                                }

                                iter = iter.AddSeconds(1); // 以秒为步长精细推进
                            }
                        }
                        else // 超过24小时查天级日志
                        {
                            isDailyMode = true;
                            DateTime iter = selectionStart.Date;
                            while (iter <= selectionEnd.Date)
                            {
                                string key = iter.ToString("yyyy-MM-dd");
                                bool hasDataThisDay = false;
                        ...
                        // ★ 核心修复：如实向用户展示底层统计后确切的起止时间边界，打消时间对不上的疑惑
                        Dispatcher.InvokeAsync(() => {
                            if (_isChartDragPaused && TxtSelectedTimeRange != null)
                            {
                                if (actualDataStart.HasValue && actualDataEnd.HasValue)
                                {
                                    DateTime displayStart = actualDataStart.Value;
                                    DateTime displayEnd = isDailyMode ? actualDataEnd.Value.AddDays(1) : actualDataEnd.Value.AddMinutes(1);
                                    TxtSelectedTimeRange.Text = $"📊 实际提取区间: {displayStart:yyyy/MM/dd HH:mm:ss} ~ {displayEnd:HH:mm:ss}";
                                }
                                else
                                {
                                    TxtSelectedTimeRange.Text = $"📊 选定区间内暂无流量记录: {selectionStart:HH:mm:ss} ~ {selectionEnd:HH:mm:ss}";
                                }
                                TxtSelectedTimeRange.Visibility = Visibility.Visible;
                            }
                            else if (!_isChartDragPaused && TxtSelectedTimeRange != null)
                            {
                                TxtSelectedTimeRange.Visibility = Visibility.Collapsed;
                            }
                        });
                        ...
                        foreach (string name in targetProcesses)
                        {
                            ...
                            if (isActive)
                            {
                                int ipCount = pAggs[name].ConnectedIPs.Count;
                                // ★ 修复编译错误：Sum 内使用 Item1 代表 Down，Item2 代表 Up
                                long totalAppSessionDown = _appIpTraffic.ContainsKey(name) ? _appIpTraffic[name].Values.Sum(v => v.Item1) : 0;
                                long totalAppSessionUp = _appIpTraffic.ContainsKey(name) ? _appIpTraffic[name].Values.Sum(v => v.Item2) : 0;

                                foreach (var ip in pAggs[name].ConnectedIPs) 
                                {
                                    long ipDown = 0, ipUp = 0;
                                    if (_appIpTraffic.ContainsKey(name) && _appIpTraffic[name].TryGetValue(ip, out var trafficRecord))
                                    {
                                        // ★ 修复编译错误：使用 Item1 和 Item2
                                        ipDown = trafficRecord.Item1;
                                        ipUp = trafficRecord.Item2;
                                    }

                                    // ★ 核心修复 Bug 2：不再平均，而是基于此 IP 在总账本中的真实占比，映射到你此时此刻圈选的时间段内显示真实比例分配！
                                    if (totalAppSessionDown > 0) ipDown = (long)(dispDown * ((double)ipDown / totalAppSessionDown));
                                    else ipDown = ipCount > 0 ? dispDown / ipCount : 0; // 极少数刚追踪还没来得及累计的，退回均分一次防0

                                    if (totalAppSessionUp > 0) ipUp = (long)(dispUp * ((double)ipUp / totalAppSessionUp));
                                    else ipUp = ipCount > 0 ? dispUp / ipCount : 0;

                                    info.Connections.Add(new IPConnectionInfo 
                                    { 
                                        IP = ip, 
                                        DownloadDisplay = FormatAdaptiveTotal(ipDown), 
                                        UploadDisplay = FormatAdaptiveTotal(ipUp) 
                                    });
                                }
                            }
                            tempUI[name] = info;
                ...
                foreach (var kv in uiDict.OrderByDescending(x => x.Value.RawTotal))
                {
                    ...
                    if (ex != null)
                    {
                        ...
                        // ★ 彻底修复 IP 流量为 0 的问题
                        var newIPs = kv.Value.Connections.ToList();
                        var newIpSet = new HashSet<string>(newIPs.Select(c => c.IP));
                        
                        // 1. 倒序安全移除已经死掉的 IP
                        for (int i = ex.Connections.Count - 1; i >= 0; i--) {
                            if (!newIpSet.Contains(ex.Connections[i].IP)) {
                                ex.Connections.RemoveAt(i);
                            }
                        }

                        // 2. 追加新 IP，或更新现有 IP 的真实流量值
                        foreach (var newIp in newIPs) {
                            var existingIp = ex.Connections.FirstOrDefault(c => c.IP == newIp.IP);
                            if (existingIp != null) {
                                existingIp.DownloadDisplay = newIp.DownloadDisplay;
                                existingIp.UploadDisplay = newIp.UploadDisplay;
                                
                                // ★ 骚操作：利用集合索引的重新赋值强行抛出 NotifyCollectionChangedAction.Replace 事件，
                                // 在不销毁对象的前提下强制 UI 重绘，杜绝因未实现 INotifyPropertyChanged 导致的死水现象
                                int index = ex.Connections.IndexOf(existingIp);
                                if (index >= 0) ex.Connections[index] = existingIp;
                            } else {
                                ex.Connections.Add(newIp); // 新增 IP 本身已带有完整 DownloadDisplay 和 UploadDisplay 数据
                            }
                        }
                    }
                ...
                // 数据分配渲染完毕，关闭 Loading 蒙层
                if (PanelProcessLoading != null) PanelProcessLoading.Visibility = Visibility.Collapsed;
                
                // ★ 如果当前正处于冻结模式，经过此次完整且精确的计算与UI覆盖后，关闭渲染锁，让画面和列表永不闪动
                if (_isChartDragPaused) _hasRenderedPausedState = true; 
            }
            catch { }
        ...
        private void TimeWindowCombo_SelectionChanged(object? s, SelectionChangedEventArgs e)
        {
            if (TimeWindowCombo?.SelectedItem is ComboBoxItem i && int.TryParse(i.Tag?.ToString(), out int m))
            {
                ...
                else
                {
                    ...
                    _currentViewMode = m;
                    _selectedTimeStartRatio = 0;
                    _selectedTimeEndRatio = 1;
                    if (PanelProcessLoading != null) PanelProcessLoading.Visibility = Visibility.Visible;
                    Dispatcher.InvokeAsync(() => ProcessTimer_Tick(null, null), DispatcherPriority.Background);
                }
            }
        ...
        private bool IsUserInactive(int seconds)
        {
